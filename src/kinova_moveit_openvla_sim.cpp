...
private:
  ...
  void process_action_with_joint_data()
  {
    ...
    //   }
      ...
      request->robot_state.joint_state.name = {"joint_1", "joint_2", "joint_3", "joint_4", "joint_5", "joint_6", "joint_7"};
      ...
      try {
          ...
          if (response->error_code.val == moveit_msgs::msg::MoveItErrorCodes::SUCCESS) {
              ...
              tf2::Quaternion current_action_data_q_;
              current_action_data_q_.setRPY(current_action_data_[3], current_action_data_[4], current_action_data_[5]);
              ...
              target_pose.orientation.x = end_effector_[3] + current_action_data_q_.x();
              target_pose.orientation.y = end_effector_[4] + current_action_data_q_.y();
              target_pose.orientation.z = end_effector_[5] + current_action_data_q_.z();
              target_pose.orientation.w = end_effector_[6] + current_action_data_q_.w();
...
int main(int argc, char * argv[])
{
  ...
  KinovaMoveItOpenvlaSim kinova_moveit_openvla_sim;
  rclcpp::executors::MultiThreadedExecutor executor;
  executor.add_node(kinova_moveit_openvla_sim.get_node());
  executor.spin();
  rclcpp::shutdown();
  return 0;
}
...
//               RCLCPP_INFO(logger_, "Computed FK position: [x: %f, y: %f, z: %f]",
//                           end_effector_position_[0], end_effector_position_[1], end_effector_position_[2]);

//               // Create a new target pose using action data
//               geometry_msgs::msg::Pose target_pose;
//               target_pose.orientation.w = 1.0;
//               target_pose.position.x = end_effector_position_[0] + current_action_data_[0];
//               target_pose.position.y = end_effector_position_[1] + current_action_data_[1];
//               target_pose.position.z = end_effector_position_[2] + current_action_data_[2];
...
//   int callback_count_;
//   std::vector<float> action_data_;
//   std::array<double, 3> end_effector_position_;
//   std::vector<double> joint_positions_;
//   // bool action_data_received_;  // Flag to track if action data was received
//   bool joint_data_received_;   // Flag to track if joint data was received
// };
...
//         // auto response = future.get();
//         // if (response->error_code.val == moveit_msgs::msg::MoveItErrorCodes::SUCCESS) {
//         //   end_effector_position_ = {response->pose_stamped[0].pose.position.x,
//         //                             response->pose_stamped[0].pose.position.y,
//         //                             response->pose_stamped[0].pose.position.z};
...
//         geometry_msgs::msg::Pose target_pose;
//         target_pose.orientation.w = 1.0;
//         target_pose.position.x = end_effector_position_[0] + action_data_[0];
//         target_pose.position.y = end_effector_position_[1] + action_data_[1];
//         target_pose.position.z = end_effector_position_[2] + action_data_[2];
